         ...
         bool isCluster = false)
      {
         ...
         for (int i = 0; i < src.Count; ++i)
         {
            try
            {
               ...
               {
                  EditorUtility.CompressTexture(normalSAOTex, GetTextureFormat(cfg, settings.normalSettings.compression), TextureCompressionQuality.Normal);
               }

               if (settings.diffuseSettings.compression != TextureArrayConfig.Compression.Uncompressed)
               {
                  EditorUtility.CompressTexture(diffuseHeightTex, GetTextureFormat(cfg, settings.diffuseSettings.compression), TextureCompressionQuality.Normal);
               }

               if (smoothAOTex != null && cfg.packingMode == TextureArrayConfig.PackingMode.Quality)
               {
                  EditorUtility.CompressTexture(smoothAOTex, GetTextureFormat(cfg, settings.smoothSettings.compression), TextureCompressionQuality.Normal);
               }

               if (antiTileTex != null && settings.antiTileSettings.compression != TextureArrayConfig.Compression.Uncompressed)
               {
                  EditorUtility.CompressTexture(antiTileTex, GetTextureFormat(cfg, settings.antiTileSettings.compression), TextureCompressionQuality.Normal);
               }
               ...
               {
                  EditorUtility.CompressTexture(emisTex, GetTextureFormat(cfg, settings.emissiveSettings.compression), TextureCompressionQuality.Normal);
               }
